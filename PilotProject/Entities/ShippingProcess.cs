...
namespace PilotProject.Entities
{
    internal sealed class ShippingProcess
    {
        ...
        {
            Thread.Sleep(60000);
            DelivMessage?.Invoke(Session.GetStatic().Email, Letter.GetTemplateLatter(Template.OrderCompletion));

            Thread.Sleep(60000);
            DelivMessage?.Invoke(Session.GetStatic().Email, Letter.GetTemplateLatter(Template.OrderDelivered));
